  ...
  methods: {
    ...
    judgeMove () {
      ...
      if (Math.abs(gap) === 1) {
        ...
        // 间隔大于1
        gap > 0 ? (max = this.endPointInfo[direction], min = this.startPointInfo[direction]) :
          (max = this.startPointInfo[direction], min = this.endPointInfo[direction]);
        // crossArr为x或y方向 起点、终点间的数组
        const crossArr = this.list.filter(d => (d.position[filterDir] === this.endPointInfo[filterDir]) &&
          (d.position[direction] > min && d.position[direction] < max));
        let isCross = true;
    ...
    move (direction) {
      ...
      setTimeout(() => {
        ...
        this.$set(
          ...
          Object.assign({},
            this.list[this.startPointActiveIndex],
            { status: "moved" },
          )
        );
        // 空格子变为冰块
        this.$set(
          ...
          Object.assign({},
            this.list[this.endPointActiveIndex],
            { status: "ice" },
          )
        );
        // 历史记录里PUSH一条
